  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Follow-up Automation Rules</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {rules.length === 0 ? (
          <div className="col-span-full bg-white dark:bg-black rounded-2xl border border-slate-200 dark:border-zinc-850 p-12 text-center text-slate-400 dark:text-zinc-500 shadow-sm">
            <Repeat className="w-8 h-8 mx-auto text-slate-300 dark:text-zinc-600 mb-2" />
            <p className="font-semibold text-slate-700 dark:text-zinc-300">No follow-up sequences active</p>
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[10px] uppercase tracking-wider font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/60 px-2.5 py-1 rounded-md border border-indigo-100 dark:border-indigo-900">
                ...
                <h3 className="font-bold text-slate-900 dark:text-white text-sm mb-1">{rule.campaignName}</h3>
                <div className="text-xs font-semibold text-slate-700 dark:text-zinc-300 truncate mb-2">
                  {rule.subjectTemplate}
                </div>

                <div className="text-xs text-slate-500 dark:text-zinc-400 line-clamp-3 bg-slate-50/80 dark:bg-zinc-900/60 p-3 rounded-xl border border-slate-100 dark:border-zinc-800 font-sans mb-3.5">
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-zinc-800 mt-4 flex justify-between items-center">
                <span className="text-[11px] text-slate-400 dark:text-zinc-500">Rule ID: {rule.id.slice(-6)}</span>
                <button
                  onClick={() => onToggleRule(rule.id)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center space-x-1 transition-colors cursor-pointer ${
                    rule.status === 'ACTIVE'
                      ? 'bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 hover:bg-amber-100 dark:hover:bg-amber-950/80 border border-amber-200 dark:border-amber-800'
                      : 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-800'
                  }`}
                >
                  {rule.status === 'ACTIVE' ? (
                    <>
      ...
      <div className="bg-white dark:bg-black rounded-2xl border border-slate-200 dark:border-zinc-850 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 dark:border-zinc-850 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold text-slate-900 dark:text-white">Pending Follow-up Instances</h2>
            <p className="text-xs text-slate-500 dark:text-zinc-400">Live tracker of queued recipient follow-ups</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 dark:bg-zinc-900 text-slate-600 dark:text-zinc-400 font-semibold uppercase tracking-wider text-[10px]">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-zinc-850">
              {instances.length === 0 ? (
                ...
                instances.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/70 dark:hover:bg-zinc-900/60 transition-colors">
                    <td className="py-3.5 px-4">
                      <div className="font-semibold text-slate-800 dark:text-zinc-200">{item.contactName}</div>
                      <div className="text-slate-500 dark:text-zinc-400 font-mono text-[11px]">{item.contactEmail}</div>
                    </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 dark:bg-black/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-black rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 dark:border-zinc-850">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Create Follow-up Cadence Rule</h3>
            ...
            <form onSubmit={handleCreateRule} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-zinc-300 mb-1">Target Campaign</label>
                  <select
                    value={selectedCampaignId}
                    onChange={(e) => setSelectedCampaignId(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-zinc-700 focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white dark:bg-zinc-900 font-medium text-slate-900 dark:text-zinc-100 cursor-pointer"
                  >
                    {campaigns.map((c) => (
                ...
                <div>
                  <label className="block font-semibold text-slate-700 dark:text-zinc-300 mb-1">Days After Previous</label>
                  <input
                    type="number"
                    min={1}
                    max={30}
              ...
              <div>
                <label className="block font-semibold text-slate-700 dark:text-zinc-300 mb-1">Subject Line Template</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block font-semibold text-slate-700 dark:text-zinc-300 mb-1">Message Body Template</label>
                <textarea
                  rows={6}
                  required
                  value={messageTemplate}
                  onChange={(e) => setMessageTemplate(e.target.value)}
                  className="w-full p-3 font-sans leading-relaxed rounded-xl border border-slate-200 dark:border-zinc-700 focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/70 dark:bg-zinc-900 text-slate-900 dark:text-zinc-100 placeholder:text-slate-400 dark:placeholder:text-zinc-500"
                />
              </div>
