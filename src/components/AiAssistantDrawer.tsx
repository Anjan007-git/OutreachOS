...
export const AiAssistantDrawer: React.FC<AiAssistantDrawerProps> = ({
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[440px] bg-white dark:bg-[#000000] shadow-2xl border-l border-slate-200 dark:border-[#171717] flex flex-col animate-in slide-in-from-right duration-200">
          {/* Drawer Header */}
          <div className="px-5 py-4 border-b border-slate-800 dark:border-[#1A1A1A] flex items-center justify-between bg-slate-900 dark:bg-[#080808] text-white">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-300 dark:text-[#A1A1AA]">Live Cold Outreach & Strategy Co-Pilot</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-1">
              <button
                ...
                className="p-1.5 text-slate-300 dark:text-[#A1A1AA] hover:text-white rounded-lg hover:bg-white/10 transition-colors cursor-pointer"
              >
          ...
          {/* Quick Prompts Bar */}
          <div className="px-4 py-2.5 bg-slate-50 dark:bg-[#050505] border-b border-slate-200 dark:border-[#161616] overflow-x-auto scrollbar-none flex items-center space-x-2">
            <span className="text-[10px] font-bold text-slate-500 dark:text-[#71717A] uppercase tracking-wider shrink-0">
              Suggestions:
            </span>
            {QUICK_PROMPTS.map((prompt, idx) => (
              <button
                ...
                className="shrink-0 px-2.5 py-1 rounded-lg bg-white dark:bg-[#0A0A0A] hover:bg-indigo-50 dark:hover:bg-[#111111] hover:text-indigo-700 dark:hover:text-[#FFFFFF] border border-slate-200 dark:border-[#1F1F1F] text-[11px] font-medium text-slate-700 dark:text-[#A1A1AA] transition-colors cursor-pointer"
              >
          ...
          {/* Chat Messages List */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-slate-50/50 dark:bg-[#000000]">
            {messages.map((m) => {
              ...
              return (
                ...
                >
                  {isAssistant && (
                    <div className="w-6 h-6 rounded-lg bg-indigo-100 dark:bg-[#0D0D0D] text-indigo-700 dark:text-indigo-400 border border-transparent dark:border-[#1C1C1C] flex items-center justify-center shrink-0 mt-1">
                  ...
                  <div
                    className={`max-w-[85%] rounded-2xl p-3.5 text-xs font-sans leading-relaxed shadow-xs ${
                      isAssistant
                        ? 'bg-white dark:bg-[#080808] border border-slate-200 dark:border-[#1A1A1A] text-slate-800 dark:text-[#FFFFFF]'
                  ...
                  >
                    ...
                    <div
                      className={`flex items-center justify-between mt-2 pt-1 text-[10px] ${
                        isAssistant ? 'text-slate-400 dark:text-[#71717A] border-t border-slate-100 dark:border-[#161616]' : 'text-indigo-200'
                      }`}
                    >
                      ...
                      {isAssistant && (
                        <div className="flex items-center space-x-2">
                          <button
                            ...
                            className="flex items-center space-x-1 hover:text-slate-700 dark:hover:text-[#FFFFFF] cursor-pointer"
                  ...
                  {!isAssistant && (
                    <div className="w-6 h-6 rounded-lg bg-slate-900 dark:bg-[#1A1A1A] text-white flex items-center justify-center shrink-0 mt-1">
            ...
            {isLoading && (
              <div className="flex items-center space-x-2.5 text-xs text-indigo-600 dark:text-indigo-300 font-medium bg-indigo-50/70 dark:bg-[#0A0A0A] p-3 rounded-xl border border-indigo-100 dark:border-[#1C1C1C]">
          ...
          {/* Chat Input Field */}
          <div className="p-4 border-t border-slate-200 dark:border-[#161616] bg-white dark:bg-[#050505]">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleSendMessage();
            ...
            >
              <input
                ...
                disabled={isLoading}
                className="flex-1 text-xs px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-[#1F1F1F] focus:ring-1 focus:ring-indigo-500 focus:outline-none bg-slate-50/50 dark:bg-[#0A0A0A] text-slate-800 dark:text-[#FFFFFF] placeholder:text-slate-400 dark:placeholder:text-[#71717A] font-medium"
              />
              <button
                type="submit"
